...
# Non-augmented train & test dataset
# (the default number of valid img is 0 so not building a valid dataset.)
for phase in ["train", "test"]:
    register_coco_instances(f'nuclei_{phase}',
                            json_file=os.path.join(
                                'dataset', f'nuclei_{phase}_dataset.json'),
                            image_root=os.path.join(
                                'dataset', f'{phase}_images'), metadata={})
...
# Uncomment the following line to use the detectron2 pretrained model
# (based on your architecture configurations)
cfg.MODEL.WEIGHTS = model_zoo.get_checkpoint_url(
    "Misc/cascade_mask_rcnn_R_50_FPN_3x.yaml")

# Train from your pretrained model weights
# Uncomment to use your own pretrained model by assigning a path
# cfg.MODEL.WEIGHTS = 'pretrained.pth'

# Other settings (You can refer to detectron2's configuration files.
...
os.makedirs(cfg.OUTPUT_DIR, exist_ok=True)  # Default output dir: output/
...
# Important!
trainer.resume_or_load(resume=False)  # Set True to resume training.
trainer.train()
